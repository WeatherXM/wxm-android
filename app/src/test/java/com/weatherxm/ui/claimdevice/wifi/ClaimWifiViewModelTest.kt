...
class ClaimWifiViewModelTest : BehaviorSpec({
    ...
    val serialNumberWithColon = "serial:Number"
    ...
    context("SET a serial number and then GET it") {
        given("a serial number") {
            ...
            and("SET it again and ensure unmasking (removing the `:`) takes place") {
                viewModel.setSerialNumber(serialNumberWithColon)
                then("GET it and ensure it's set correctly") {
                    viewModel.getSerialNumber() shouldNotBe serialNumberWithColon
                    viewModel.getSerialNumber() shouldBe serial
                }
            }
        }
    ...
    context("Validate serial number and claiming key") {
        given("A serial number") {
            ...
            When("it's valid") {
                then("return true") {
                    viewModel.validateSerial(validSerialNumberD1) shouldBe true
                }
            }
        }
        given("A claiming key") {
            ...
            When("it's valid") {
                then("return true") {
                    viewModel.validateClaimingKey(validClaimingKeyD1) shouldBe true
                }
            }
        }
    }
